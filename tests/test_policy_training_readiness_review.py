...
class PolicyTrainingReadinessReviewTests(unittest.TestCase):
    ...
    def test_quasi_real_guarded_formal_ppo_stability_holdout_summary_advances_readiness(self) -> None:
        stability_path = (
            self.batch_root
        ...
        stability_path.write_text(
            json.dumps(
                self._formal_stability_holdout_summary(),
                indent=2,
            ),
            encoding="utf-8",
        )
    ...
    def test_quasi_real_guarded_formal_ppo_stability_holdout_regression_blocks_readiness(self) -> None:
        stability_path = (
            self.batch_root
            / "quasi-real-guarded-formal-ppo-stability-holdout-validation-summary.json"
        )
        payload = self._formal_stability_holdout_summary()
        payload["controlled_regression_count"] = 1
        payload["validation_controlled_regression_count"] = 1
        ...
        self.assertIn(
            "quasi_real_guarded_formal_ppo_stability_holdout_controlled_regression",
            summary["training_blockers"],
        )
    ...
    def test_guarded_experimental_policy_staged_release_preflight_advances_readiness(self) -> None:
        preflight_path = (
            self.batch_root
        ...
        preflight_path.write_text(
            json.dumps(
                self._guarded_experimental_policy_staged_release_preflight_summary(),
                indent=2,
            ),
            encoding="utf-8",
        )
        ...
        self.assertEqual(summary["reason_codes"], [])
        self.assertTrue(
            summary["guarded_experimental_policy_staged_release_preflight_readiness"][
                "completed"
            ]
        )

    def test_guarded_experimental_policy_staged_release_preflight_boundary_blocks_readiness(self) -> None:
        preflight_path = (
            self.batch_root
            / "guarded-experimental-policy-staged-release-preflight-summary.json"
        )
        payload = self._guarded_experimental_policy_staged_release_preflight_summary()
        payload["staged_release_preflight_verdict"] = "blocked_by_staged_release_preflight"
        payload["staged_release_enabled"] = True
        payload["experimental_control_activation_count"] = 1
        payload["controlled_regression_count"] = 1
        payload["telemetry_audit_passed"] = False
        preflight_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
        ...
        self.assertIn(
            "guarded_experimental_policy_staged_release_preflight_telemetry_failed",
            summary["training_blockers"],
        )
    ...
    def test_guarded_formal_ppo_training_run_advances_readiness(self) -> None:
        training_run_path = self.batch_root / "formal-ppo-training-run-summary.json"
        training_run_path.write_text(
            json.dumps(self._guarded_formal_ppo_training_run_summary(), indent=2),
            encoding="utf-8",
        )
        ...
        self.assertEqual(summary["training_blockers"], [])
        self.assertEqual(summary["reason_codes"], [])
        self.assertTrue(summary["guarded_formal_ppo_training_run_readiness"]["completed"])

    def test_guarded_formal_ppo_training_run_boundary_blocks_readiness(self) -> None:
        training_run_path = self.batch_root / "formal-ppo-training-run-summary.json"
        payload = self._guarded_formal_ppo_training_run_summary()
        payload["passed_seed_count"] = 4
        payload["loss_non_finite_count"] = 1
        payload["controlled_path_risk_regression_count"] = 1
        payload["post_training_holdout_status"] = "failed"
        payload["publishes_checkpoint"] = True
        training_run_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
        ...
        self.assertIn(
            "guarded_formal_ppo_training_run_seed_not_all_passed",
            summary["training_blockers"],
        )
        self.assertIn("ppo_update_loss_non_finite", summary["training_blockers"])
        self.assertIn(
            "guarded_formal_ppo_training_run_controlled_regression",
            summary["training_blockers"],
        )
        self.assertIn(
            "guarded_formal_ppo_training_run_post_training_gate_failed",
            summary["training_blockers"],
        )
        self.assertIn(
            "limited_ppo_update_checkpoint_publication_claimed",
    ...
    def _guarded_experimental_policy_staged_release_preflight_summary(self) -> dict:
        return {
            ...
            "source_shadow_release_trial_summary": (
    ...
    def _guarded_formal_ppo_training_run_summary(self) -> dict:
        return {
