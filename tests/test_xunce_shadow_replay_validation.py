...
class XunceShadowReplayValidationTests(unittest.TestCase):
    def setUp(self) -> None:
        self.repo_root = Path(tempfile.mkdtemp(prefix="xunce-shadow-replay-"))
        ...
        self.output_root = self.repo_root / "outputs" / "stage15"
        ...
        self.config_path = self.repo_root / "configs" / "xunce_shadow_replay_validation_v1.json"
    ...
    def test_default_fixture_replays_and_matches_source(self) -> None:
        ...
        self.assertEqual(summary["next_required_change"], "sandbox_candidate_preflight")
        self.assertTrue(summary["shadow_replay_passed"])
        self.assertTrue(summary["source_match_audit_passed"])
        self.assertTrue(summary["replay_determinism_passed"])
    ...
    def test_missing_post_training_source_routes_to_fix(self) -> None:
        from scripts.run_xunce_shadow_replay_validation import run_xunce_shadow_replay_validation

        shutil.rmtree(self.post_training_root)
        summary = run_xunce_shadow_replay_validation(
            config_path=self.config_path,
            output_root=self.output_root,
            repo_root=self.repo_root,
        ...
        self.assertIn("missing_post_training_summary", summary["reason_codes"])
    ...
    def test_source_match_mismatch_routes_to_determinism_fix(self) -> None:
        ...
        rows_path = self.post_training_root / "xunce-post-training-offline-evaluation-results.jsonl"
        rows = [json.loads(line) for line in rows_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        rows[0]["trained_target_probability"] += 0.25
        ...
        self.assertEqual(summary["next_required_change"], "fix_xunce_shadow_replay_determinism")
        self.assertGreater(summary["scenario_mismatch_count"], 0)

    def test_source_boundary_violation_blocks_replay(self) -> None:
        from scripts.run_xunce_shadow_replay_validation import run_xunce_shadow_replay_validation

        summary_path = self.post_training_root / "xunce-post-training-offline-evaluation-summary.json"
        ...
        self.assertEqual(summary["status"], "failed")
        self.assertIn("shadow_replay_source_boundary_violation", summary["reason_codes"])
        self.assertEqual(summary["next_required_change"], "resolve_xunce_shadow_replay_boundary_rejections")
        self.assertFalse(summary["checkpoint_loaded"])
