...
class PathPlannerAlgorithmReportTests(unittest.TestCase):
    def test_report_tracks_gcs_control_point_direction_cone_stage(self) -> None:
        ...
        self.assertIn("anchor-projection-evidence-contract-summary/v1", content)
        self.assertIn("current_git_provenance_mismatch_count=0", content)
        self.assertIn("git_provenance_mismatch_count=0", content)
        self.assertIn("positive_training_evidence_contains_audit_proxy_anchor_count=0", content)
        self.assertIn("keep_platform_blocked_targets_out_of_training", content)
        self.assertIn("不启动 PPO", content)
