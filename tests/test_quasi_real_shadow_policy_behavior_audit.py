...
class QuasiRealShadowPolicyBehaviorAuditTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temp_dir = Path(tempfile.mkdtemp(prefix="quasi-real-shadow-audit-"))
    ...
    def test_shadow_audit_passes_for_aligned_and_gate_passed_policy_changes(self) -> None:
        from scripts.run_quasi_real_shadow_policy_behavior_audit import (
        ...
        self.assertEqual(summary["behavior_verdict"], "acceptable_for_quasi_real_guarded_pilot")
        self.assertEqual(summary["shadow_context_count"], 2)
        self.assertEqual(summary["policy_decision_count"], 2)
        self.assertEqual(summary["policy_source_aligned_count"], 1)
        ...
        self.assertFalse(summary["policy_takes_control"])
        self.assertTrue((self.output_root / "quasi-real-shadow-policy-decisions.jsonl").is_file())
        self.assertTrue((self.output_root / "quasi-real-shadow-policy-group-report.md").is_file())

    def test_shadow_audit_rejects_policy_changed_gate_regression_with_report(self) -> None:
        from scripts.run_quasi_real_shadow_policy_behavior_audit import (
            run_quasi_real_shadow_policy_behavior_audit,
        )
        ...
        self.assertEqual(summary["behavior_verdict"], "policy_real_map_alignment_refinement_required")
        self.assertIn("quasi_real_shadow_gate_regression", summary["reason_codes"])
        self.assertEqual(summary["policy_changed_gate_rejected_count"], 1)
        self.assertEqual(summary["path_cost_regression_count"], 1)
        report = json.loads(
            (self.output_root / "quasi-real-shadow-policy-rejection-report.json").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(report["rejected_count"], 1)
        self.assertEqual(report["rejections"][0]["gate_reason_codes"], ["path_cost_regression"])

    def test_readiness_accepts_passed_shadow_summary(self) -> None:
        from scripts.run_policy_training_readiness_review import (
            _quasi_real_shadow_policy_behavior_readiness,
        )

        readiness = _quasi_real_shadow_policy_behavior_readiness(
            {
    ...
    def test_readiness_blocks_failed_shadow_summary(self) -> None:
        from scripts.run_policy_training_readiness_review import (
            _quasi_real_shadow_policy_behavior_readiness,
        )
        ...
        readiness = _quasi_real_shadow_policy_behavior_readiness(
            {
                ...
                "replaces_default_policy": False,
                "performance_claimed": False,
                "next_required_change": "policy_real_map_alignment_refinement_required",
            }
        )

        self.assertFalse(readiness["completed"])
        self.assertIn(
            "quasi_real_shadow_policy_alignment_refinement_required",
            readiness["training_blockers"],
        )
        self.assertIn("quasi_real_shadow_gate_regression", readiness["training_blockers"])
    ...
    def _scenario(self, scenario_id: str, group: str) -> dict[str, object]:
        return {
            ...
            "path_feedback": {
                "candidates": [
                    self._candidate(scenario_id, group, 0, "source_selected", 10.0),
                    self._candidate(scenario_id, group, 1, "not_source_candidate", 9.0),
                ]
            },
        }
    ...
    def _config(self) -> dict[str, object]:
        return {
            ...
            "evaluation": {"max_path_cost_regression": 0.0, "max_risk_regression": 0.0},
