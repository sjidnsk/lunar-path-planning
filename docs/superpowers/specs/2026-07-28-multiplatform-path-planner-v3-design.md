...
- 飞跃式平台：单个确定性的下一着陆状态，以及本次飞跃的边界条件。
...
├── thrust_bounds
├── attitude_bounds
├── fuel_or_impulse_bounds
├── flight_corridor_3d
├── landing_position_bounds
├── landing_velocity_bounds
├── landing_attitude_bounds
└── post_landing_hold_condition
```

内部可以使用不确定性估计筛选候选，但公共结果保持确定、简单和可验证。
...
共享层在路线骨架前方生成已知安全着陆区域。平台层计算以下集合的交集：

\[
\mathcal L =
\mathcal L_{\mathrm{reachable}}
\cap
\mathcal L_{\mathrm{safe}}
\cap
\mathcal L_{\mathrm{visible}}
\cap
\mathcal L_{\mathrm{mission}}.
\]

其中分别表示动力学可达、几何安全、感知可信和任务有效的着陆集合。

### 9.2 确定性目标选择

从交集中筛选 Top-K 候选，对每个候选执行有界飞行可行性检查。排序优先级为：
...
\[
X_{\mathrm{landing}}=(x,y,z,\psi).
\]

若没有通过完整验证的候选，则不发布新的飞跃命令。
...
若平台已经起飞，重规划不得中断已承诺飞行段，只能更新着陆后的保持或后续任务建议。
...
新规划必须从旧的承诺边界连续接续，不能在执行中瞬间改变已承诺路径、速度或飞跃目标。
