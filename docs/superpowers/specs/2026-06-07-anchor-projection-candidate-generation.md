...
## Anchor-Reachability-Aware Candidate Generation v1

本阶段把 anchor projection 候选生成从“几何最近优先”升级为“连通可达优先 + 质量排序”。
实现位置为 `model-explorer/src/model_explorer/policy/planning.py`：

- 对 inflated passable mask 做连通分量标注，记录 start、target、nearest anchor 与 projected
  anchor 的 component id 和 component size。
- 保留全图几何最近 anchor 作为诊断字段；若该 anchor 不可达，则在 start 所在连通分量内搜索
  reachable substitute anchor。
- 保留 `require_anchor_reachable=True`，不可达 anchor 不会生成正样本。
- candidate-generation summary、evidence-contract summary 与 readiness review 均透传
  `reachable_substitute_anchor_found_count`、
  `anchor_unreachable_repaired_by_reachable_substitute_count` 和
  `true_geometry_unreachable_count`。
- candidate-generation 的训练正样本口径显式对齐 evidence contract：
  `max_trainable_projection_distance_m=1.0`，
  `max_trainable_projection_distance_cells=2`。超过该距离的 source-selected substitute
  只作为诊断候选，不计入 trainable。
...
- candidate-generation：`status=passed`，`reason_codes=[]`，
  ...
  `source_selection_quality_regression_count=0`。
- 原 36 个 `anchor_unreachable` 已全部被解释为
  `reachable_substitute_anchor_found_count=36`，
  `anchor_unreachable_repaired_by_reachable_substitute_count=36`，
  `true_geometry_unreachable_count=0`。
- 36 个 reachable substitute 均超过当前训练距离 contract；
  其中 12 个被 source selection 选中但因距离超过 2 cells / 1.0 m 不计入训练正例。
- evidence-contract：`status=passed`，`contract_blockers=[]`，
  `candidate_contract_alignment_gap_count=0`。
- policy readiness：`training_readiness_status=needs_training_contract_refinement`，
  `training_blockers=["anchor_projection_nontrainable_contexts_remain"]`，
  `anchor_projection_readiness_trainable_count=18`。

结论：36 个 `anchor_unreachable` 不是“样本完全不可达”，而是旧策略只看几何最近 anchor
导致的候选生成漏选。算法已能找出可达替代 anchor，但这些替代 anchor 往往距离原目标过远；
下一阶段应处理 source-selection 质量排序与训练距离 contract 的取舍，仍不启动 PPO。
