...
SRC="${SRC:-outputs/path_feedback_batch_value_stability_clean_src_v1}"
BASE="${BASE:-outputs/path_feedback_batch_value_stability_baseline_candidate_v1}"
DEV="${DEV:-outputs/path_feedback_batch_value_stability_dev_v1}"
TRAIN="${TRAIN:-outputs/path_feedback_batch_value_stability_train_v1}"
VAL="${VAL:-outputs/path_feedback_batch_value_stability_val_v1}"
TEST="${TEST:-outputs/path_feedback_batch_value_stability_test_v1}"
CAND="${CAND:-outputs/path_feedback_batch_value_stability_candidate_v1}"
CANARY="${CANARY:-outputs/path_feedback_batch_policy_gated_canary_value_stability_v1}"
...
PYTHON="$PYTHON_BIN" bash "$SCRIPT_DIR/run_batch_path_feedback_validation.sh" \
  --matrix "$REPO_ROOT/configs/path_feedback_batch_policy_gated_canary_value_stability_v1.json"

PYTHON="$PYTHON_BIN" bash "$SCRIPT_DIR/run_policy_gated_canary_rollout.sh" \
  --source-root "$SRC" \
  --candidate-root "$CAND" \
  --batch-root "$CANARY" \
  --config "$REPO_ROOT/configs/policy_gated_canary_value_stability_v1.json"
