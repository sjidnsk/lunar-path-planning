...
"${PYTHON}" "${REPO_ROOT}/scripts/run_refined_coverage_reward_margin_tuning.py" \
  --refined-root "${REPO_ROOT}/outputs/path_feedback_batch_refined_coverage_driven_ppo_improvement_run_v2" \
  ...
  --output-root "${REPO_ROOT}/outputs/path_feedback_batch_refined_coverage_reward_margin_tuning_v1" \
  --repo-root "${REPO_ROOT}"
