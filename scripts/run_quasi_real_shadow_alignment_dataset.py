...
SUMMARY_SCHEMA_VERSION = "quasi-real-shadow-alignment-split-summary/v1"
SLICE_SCHEMA_VERSION = "quasi-real-shadow-alignment-slice/v1"
DEFAULT_OUTPUT_ROOT = "outputs/path_feedback_batch_quasi_real_shadow_alignment_dataset_v1"


def run_quasi_real_shadow_alignment_dataset(
    *,
    ...
    variants = config.get("variants", {"train": 2, "val": 2, "holdout": 2})
    for failure_index, record in enumerate(records):
        for split, count in variants.items():
            for variant_index in range(int(count)):
                slices.append(_variant_slice(record, failure_index, split, variant_index))
    ...
    for split in ("train", "val", "holdout"):
        if sum(1 for item in slices if item.get("split") == split) <= 0:
            reason_codes.append(f"{split}_split_empty")
    if overlap["context_id_overlap_count"]:
        reason_codes.append("quasi_real_shadow_alignment_context_leakage_detected")
    if overlap["scenario_id_overlap_count"] or overlap["slice_id_overlap_count"]:
        reason_codes.append("quasi_real_shadow_alignment_context_leakage_detected")

    path_feedback = {
        "schema_version": "path-feedback-summary/v1",
        "scenario_count": len(slices),
        "scenarios": [_scenario_from_slice(item, records) for item in slices],
    }
    summary = {
        ...
        "val_slice_count": sum(1 for item in slices if item.get("split") == "val"),
        "holdout_slice_count": sum(1 for item in slices if item.get("split") == "holdout"),
        **overlap,
        "next_required_change": None if not reason_codes else "quasi_real_shadow_alignment_context_leakage_detected",
...
def _variant_slice(record: dict[str, Any], failure_index: int, split: str, variant_index: int) -> dict[str, Any]:
    base = f"{record.get('scenario_id')}:{split}:{variant_index}"
    scenario_id = f"qreal_shadow_align_{failure_index:02d}_{split}_{variant_index:02d}"
    return {
        "schema_version": SLICE_SCHEMA_VERSION,
        "scenario_id": scenario_id,
        "scenario_group": record.get("roi_group", "mixed_risk"),
        "scenario_seed": 20260610 + failure_index * 100 + variant_index,
        "scenario_variant_id": f"{scenario_id}-variant",
        "dataset_id": "lunar_south_pole_lro_lola_gdr_875s_20m",
        "data_class": "quasi_real_shadow_alignment",
...
def _scenario_from_slice(slice_record: dict[str, Any], records: list[dict[str, Any]]) -> dict[str, Any]:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate quasi-real shadow alignment split variants.")
