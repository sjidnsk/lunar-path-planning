...
QUASI_REAL_GUARDED_FORMAL_PPO_CANDIDATE_SELECTION_LONG_HORIZON_HOLDOUT_EVALUATED_ACTION = (
...
GUARDED_EXPERIMENTAL_POLICY_RELEASE_CANDIDATE_PACKAGING_EVALUATED_ACTION = (
...
GUARDED_EXPERIMENTAL_POLICY_SHADOW_RELEASE_TRIAL_EVALUATED_ACTION = (
...
GUARDED_EXPERIMENTAL_POLICY_STAGED_RELEASE_TRIAL_EVALUATED_ACTION = (
    "guarded_experimental_policy_staged_release_trial_evaluated"
)
GUARDED_EXPERIMENTAL_POLICY_STAGED_RELEASE_TRIAL_SCHEMA_VERSION = (
    "guarded-experimental-policy-staged-release-trial-summary/v1"
)
GUARDED_EXPERIMENTAL_POLICY_STAGED_RELEASE_CANARY_PREFLIGHT_EVALUATED_ACTION = (
...
GUARDED_FORMAL_PPO_POST_TRAINING_STABILITY_REPLAY_EVALUATED_ACTION = (
    "guarded_formal_ppo_post_training_stability_replay_evaluated"
)
GUARDED_FORMAL_PPO_POST_TRAINING_STABILITY_REPLAY_SCHEMA_VERSION = (
    "guarded-formal-ppo-post-training-stability-replay-summary/v1"
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Optional guarded-experimental-policy-staged-release-trial-summary/v1 JSON.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Optional guarded-formal-ppo-post-training-stability-replay-summary/v1 JSON.",
    )
    parser.add_argument(
    ...
    quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path = (
        _resolve_path(
    ...
    guarded_experimental_policy_release_candidate_packaging_path = (
        _resolve_path(
            args.guarded_experimental_policy_release_candidate_packaging_summary,
            repo_root,
        )
    ...
    guarded_experimental_policy_shadow_release_trial_path = (
        _resolve_path(
            args.guarded_experimental_policy_shadow_release_trial_summary,
            repo_root,
        )
    ...
    guarded_experimental_policy_staged_release_trial_path = (
        _resolve_path(
            args.guarded_experimental_policy_staged_release_trial_summary,
            repo_root,
        )
        if args.guarded_experimental_policy_staged_release_trial_summary
        else batch_root / "guarded-experimental-policy-staged-release-trial-summary.json"
    )
    guarded_experimental_policy_staged_release_canary_preflight_path = (
        _resolve_path(
            args.guarded_experimental_policy_staged_release_canary_preflight_summary,
            repo_root,
        )
    ...
    guarded_formal_ppo_post_training_stability_replay_path = (
        _resolve_path(
            args.guarded_formal_ppo_post_training_stability_replay_summary,
            repo_root,
        )
        if args.guarded_formal_ppo_post_training_stability_replay_summary
        else batch_root / "formal-ppo-post-training-stability-replay-summary.json"
    )
    ...
    explicit_quasi_real_guarded_formal_candidate_selection_long_horizon_only_summary = bool(
        args.quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_summary
    ) and not any(
        (
            args.anchor_projection_candidate_generation_summary,
    ...
    anchor_only_defaults_available = (
        ...
        and not explicit_quasi_real_guarded_formal_candidate_selection_long_horizon_only_summary
    ...
    if args.guarded_formal_ppo_post_training_stability_replay_summary:
        summary = _analyze_guarded_formal_ppo_post_training_stability_replay_stage_only(
            batch_root=batch_root,
            replay_path=guarded_formal_ppo_post_training_stability_replay_path,
            config=config,
            repo_root=repo_root,
        )
        ...
        validation_message = {
            ...
            "guarded_formal_ppo_post_training_stability_replay_summary": _display_path(
                guarded_formal_ppo_post_training_stability_replay_path,
                repo_root,
            ),
            ...
            "guarded_formal_ppo_post_training_stability_replay_readiness": summary[
                "guarded_formal_ppo_post_training_stability_replay_readiness"
            ],
    ...
    if args.guarded_experimental_policy_staged_release_canary_preflight_summary:
        summary = _analyze_guarded_experimental_policy_staged_release_canary_preflight_stage_only(
            batch_root=batch_root,
            canary_preflight_path=guarded_experimental_policy_staged_release_canary_preflight_path,
            config=config,
            repo_root=repo_root,
        )
        output_file = _output_file(batch_root, config)
        validation_message = {
            "status": "config validated" if summary["status"] == "passed" else "validation failed",
            "batch_root": _display_path(batch_root, repo_root),
            "guarded_experimental_policy_staged_release_canary_preflight_summary": _display_path(
                guarded_experimental_policy_staged_release_canary_preflight_path,
                repo_root,
            ),
            "config": _display_path(config_path, repo_root),
            ...
            "training_blockers": summary["training_blockers"],
            "recommended_next_action": summary["recommended_next_action"],
            "guarded_experimental_policy_staged_release_canary_preflight_readiness": summary[
                "guarded_experimental_policy_staged_release_canary_preflight_readiness"
            ],
            "policy_training_readiness_review_summary": _display_path(output_file, repo_root),
        }
        print(json.dumps(validation_message, ensure_ascii=False))
    ...
    if args.guarded_experimental_policy_staged_release_trial_summary:
        summary = _analyze_guarded_experimental_policy_staged_release_trial_stage_only(
            batch_root=batch_root,
            staged_trial_path=guarded_experimental_policy_staged_release_trial_path,
            config=config,
            repo_root=repo_root,
        )
        output_file = _output_file(batch_root, config)
        validation_message = {
            "status": "config validated" if summary["status"] == "passed" else "validation failed",
            "batch_root": _display_path(batch_root, repo_root),
            "guarded_experimental_policy_staged_release_trial_summary": _display_path(
                guarded_experimental_policy_staged_release_trial_path,
                repo_root,
            ),
            "config": _display_path(config_path, repo_root),
            ...
            "training_blockers": summary["training_blockers"],
            "recommended_next_action": summary["recommended_next_action"],
            "guarded_experimental_policy_staged_release_trial_readiness": summary[
                "guarded_experimental_policy_staged_release_trial_readiness"
            ],
            "policy_training_readiness_review_summary": _display_path(output_file, repo_root),
        }
        print(json.dumps(validation_message, ensure_ascii=False))
    ...
    if args.guarded_experimental_policy_shadow_release_trial_summary:
        summary = _analyze_guarded_experimental_policy_shadow_release_trial_stage_only(
            batch_root=batch_root,
            shadow_trial_path=guarded_experimental_policy_shadow_release_trial_path,
            config=config,
            repo_root=repo_root,
        )
        output_file = _output_file(batch_root, config)
        validation_message = {
            "status": "config validated" if summary["status"] == "passed" else "validation failed",
            "batch_root": _display_path(batch_root, repo_root),
            "guarded_experimental_policy_shadow_release_trial_summary": _display_path(
                guarded_experimental_policy_shadow_release_trial_path,
                repo_root,
            ),
            "config": _display_path(config_path, repo_root),
            ...
            "training_blockers": summary["training_blockers"],
            "recommended_next_action": summary["recommended_next_action"],
            "guarded_experimental_policy_shadow_release_trial_readiness": summary[
                "guarded_experimental_policy_shadow_release_trial_readiness"
            ],
            "policy_training_readiness_review_summary": _display_path(output_file, repo_root),
        }
        print(json.dumps(validation_message, ensure_ascii=False))
    ...
    if args.guarded_experimental_policy_release_candidate_packaging_summary:
        summary = _analyze_guarded_experimental_policy_release_candidate_packaging_stage_only(
            batch_root=batch_root,
            packaging_path=guarded_experimental_policy_release_candidate_packaging_path,
            config=config,
            repo_root=repo_root,
        )
        output_file = _output_file(batch_root, config)
        validation_message = {
            "status": "config validated" if summary["status"] == "passed" else "validation failed",
            "batch_root": _display_path(batch_root, repo_root),
            "guarded_experimental_policy_release_candidate_packaging_summary": _display_path(
                guarded_experimental_policy_release_candidate_packaging_path,
                repo_root,
            ),
            "config": _display_path(config_path, repo_root),
            ...
            "training_blockers": summary["training_blockers"],
            "recommended_next_action": summary["recommended_next_action"],
            "guarded_experimental_policy_release_candidate_packaging_readiness": summary[
                "guarded_experimental_policy_release_candidate_packaging_readiness"
            ],
            "policy_training_readiness_review_summary": _display_path(output_file, repo_root),
        }
        print(json.dumps(validation_message, ensure_ascii=False))
    ...
    summary = analyze_policy_training_readiness_review(
        ...
        quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_required=bool(
            args.quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_summary
        ),
    ...
    validation_message = {
        ...
        "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_summary": (
            _display_path(
                quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path,
                repo_root,
            )
            if quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path.is_file()
            or args.quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_summary
            else None
        ),
        ...
        "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_readiness": summary.get(
...
def _analyze_guarded_experimental_policy_release_candidate_packaging_stage_only(
    *,
    batch_root: Path,
    packaging_path: Path,
    config: dict[str, Any],
    ...
    reason_codes: list[str] = []
    source_summaries: dict[str, Any] = {}
    packaging_summary = _load_source(
        packaging_path,
        label="guarded_experimental_policy_release_candidate_packaging_summary",
        expected_schema=GUARDED_EXPERIMENTAL_POLICY_RELEASE_CANDIDATE_PACKAGING_SCHEMA_VERSION,
        repo_root=repo_root,
        reason_codes=reason_codes,
        source_summaries=source_summaries,
    )
    ...
    readiness = _guarded_experimental_policy_release_candidate_packaging_readiness(
        packaging_summary
    )
    blockers = list(readiness["training_blockers"])
    training_readiness_status = (
    ...
    return {
        ...
        "training_blockers": blockers,
        "recommended_next_action": recommended_next_action,
        "guarded_experimental_policy_release_candidate_packaging_readiness": readiness,
        "git_provenance": {"current": current_git, "current_matches_sources": not reason_codes},
    }


def _guarded_experimental_policy_release_candidate_packaging_readiness(
    ...
    if summary.get("status") != "passed" or _string_list(summary.get("reason_codes")):
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_not_passed",
        )
    if summary.get("package_verdict") != "eligible_for_guarded_install_dry_run":
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_not_eligible",
        )
    if not summary.get("decision_review_summary"):
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_decision_missing",
        )
    for field in (
        "package_manifest",
        ...
        if not summary.get(field):
            _append_reason(
                blockers,
                "guarded_experimental_policy_release_candidate_packaging_artifacts_missing",
            )
    checkpoint_sha256 = summary.get("checkpoint_sha256")
    package_checkpoint_sha256 = summary.get("package_checkpoint_sha256")
    checkpoint_size = _int_value_or_default(summary.get("checkpoint_size_bytes"), 0)
    package_checkpoint_size = _int_value_or_default(
        summary.get("package_checkpoint_size_bytes"),
        0,
    )
    if (
        not isinstance(checkpoint_sha256, str)
        or len(checkpoint_sha256) != 64
        or checkpoint_sha256 != package_checkpoint_sha256
    ):
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_checkpoint_hash_mismatch",
        )
    if checkpoint_size <= 0 or checkpoint_size != package_checkpoint_size:
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_checkpoint_size_mismatch",
        )
    if summary.get("checkpoint_identity_audit_passed") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_checkpoint_identity_failed",
        )
    if summary.get("checkpoint_load_passed") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_checkpoint_load_failed",
        )
    if _int_value_or_default(summary.get("checkpoint_load_sample_count"), 0) < 64:
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_load_sample_below_threshold",
        )
    for field, reason in (
        ("invalid_action_mask_count", "guarded_experimental_policy_release_candidate_packaging_invalid_action_mask"),
        ("missing_observation_count", "guarded_experimental_policy_release_candidate_packaging_missing_observation"),
        ("non_finite_logits_count", "guarded_experimental_policy_release_candidate_packaging_non_finite_inference"),
        ("non_finite_log_prob_count", "guarded_experimental_policy_release_candidate_packaging_non_finite_inference"),
        ("non_finite_value_count", "guarded_experimental_policy_release_candidate_packaging_non_finite_inference"),
    ):
    ...
    if summary.get("runs_release_candidate_packaging") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_not_run",
        )
    if summary.get("runs_new_ppo_update") is True:
        _append_reason(blockers, "formal_ppo_update_unexpected")
    if summary.get("executes_install_or_canary") is True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_release_candidate_packaging_install_or_canary_claimed",
        )
    ...
    return {
        ...
        "package_verdict": summary.get("package_verdict"),
        ...
        "checkpoint_load_sample_count": _int_value_or_default(
            summary.get("checkpoint_load_sample_count"),
            0,
        ),
...
def _analyze_guarded_experimental_policy_staged_release_canary_preflight_stage_only(
    *,
    batch_root: Path,
    canary_preflight_path: Path,
    config: dict[str, Any],
    repo_root: Path,
) -> dict[str, Any]:
    reason_codes: list[str] = []
    source_summaries: dict[str, Any] = {}
    canary_preflight_summary = _load_source(
        canary_preflight_path,
        label="guarded_experimental_policy_staged_release_canary_preflight_summary",
        expected_schema=GUARDED_EXPERIMENTAL_POLICY_STAGED_RELEASE_CANARY_PREFLIGHT_SCHEMA_VERSION,
        repo_root=repo_root,
        reason_codes=reason_codes,
        source_summaries=source_summaries,
    )
    current_git = _git_snapshot(repo_root)
    if canary_preflight_summary:
        _inspect_git(
    ...
    readiness = _guarded_experimental_policy_staged_release_canary_preflight_readiness(
        canary_preflight_summary
    )
    blockers = list(readiness["training_blockers"])
    training_readiness_status = (
...
def _analyze_guarded_formal_ppo_post_training_stability_replay_stage_only(
    *,
    batch_root: Path,
    replay_path: Path,
    config: dict[str, Any],
    ...
    reason_codes: list[str] = []
    source_summaries: dict[str, Any] = {}
    replay_summary = _load_source(
        replay_path,
        label="guarded_formal_ppo_post_training_stability_replay_summary",
        expected_schema=GUARDED_FORMAL_PPO_POST_TRAINING_STABILITY_REPLAY_SCHEMA_VERSION,
        repo_root=repo_root,
        reason_codes=reason_codes,
        source_summaries=source_summaries,
    )
    ...
    readiness = _guarded_formal_ppo_post_training_stability_replay_readiness(replay_summary)
    blockers = list(readiness["training_blockers"])
    training_readiness_status = (
        GUARDED_FORMAL_PPO_POST_TRAINING_STABILITY_REPLAY_EVALUATED_ACTION
        if not reason_codes and readiness["completed"]
    ...
    recommended_next_action = (
        ...
        if training_readiness_status
        == GUARDED_FORMAL_PPO_POST_TRAINING_STABILITY_REPLAY_EVALUATED_ACTION
        else "fix_guarded_formal_ppo_post_training_stability_replay"
    )
    return {
        "schema_version": SUMMARY_SCHEMA_VERSION,
        "generated_at": _utc_now(),
        ...
        "training_blockers": blockers,
        "recommended_next_action": recommended_next_action,
        "guarded_formal_ppo_post_training_stability_replay_readiness": readiness,
        "git_provenance": {"current": current_git, "current_matches_sources": not reason_codes},
    }


def _guarded_formal_ppo_post_training_stability_replay_readiness(
    summary: dict[str, Any],
) -> dict[str, Any]:
    empty = {
        "present": False,
    ...
    replay_count_per_seed = _int_value_or_default(summary.get("replay_count_per_seed"), 0)
    total_replay_count = _int_value_or_default(summary.get("total_replay_count"), 0)
    ...
    replay_trainable = _int_value_or_default(
        summary.get("replay_collector_trainable_transition_count"),
        0,
    )
    ...
    if summary.get("status") != "passed" or _string_list(summary.get("reason_codes")):
        _append_reason(
            blockers,
            "guarded_formal_ppo_post_training_stability_replay_not_passed",
        )
    if summary.get("input_formal_training_run_status") != "passed":
        _append_reason(blockers, "guarded_formal_ppo_post_training_source_not_passed")
    if (
        summary.get("input_formal_training_run_readiness_status")
        != GUARDED_FORMAL_PPO_TRAINING_RUN_EVALUATED_ACTION
    ):
        _append_reason(blockers, "guarded_formal_ppo_post_training_source_readiness_invalid")
    if summary.get("runs_guarded_formal_ppo_post_training_stability_replay") is not True:
        _append_reason(blockers, "guarded_formal_ppo_post_training_stability_replay_not_run")
    if summary.get("runs_new_ppo_update") is True:
        _append_reason(blockers, "formal_ppo_update_unexpected")
    if seed_count < 5 or replay_count_per_seed < 3 or total_replay_count < 15:
        _append_reason(
            blockers,
            "guarded_formal_ppo_post_training_stability_replay_count_below_threshold",
        )
    if passed_replay_count != total_replay_count:
        _append_reason(
            blockers,
            "guarded_formal_ppo_post_training_stability_replay_not_all_passed",
        )
    if _int_value_or_default(summary.get("missing_seed_candidate_checkpoint_count"), 0):
        _append_reason(blockers, "missing_seed_candidate_checkpoint")
    if _int_value_or_default(summary.get("replay_behavior_drift_count"), 0):
        _append_reason(
            blockers,
    ...
    if optimizer_trainable < 684:
        _append_reason(
            blockers,
            "guarded_formal_ppo_post_training_stability_replay_trainable_count_below_threshold",
        )
    if replay_trainable < 684:
        _append_reason(
            blockers,
            "guarded_formal_ppo_post_training_stability_replay_trainable_count_below_threshold",
        )
    ...
    for field, reason in (
        ("validation_trainable_count", "guarded_formal_ppo_post_training_stability_replay_split_leakage"),
        ("test_trainable_count", "guarded_formal_ppo_post_training_stability_replay_split_leakage"),
        ("fallback_trainable_count", "guarded_formal_ppo_post_training_stability_replay_fallback_trainable"),
        ("source_fallback_trainable_count", "guarded_formal_ppo_post_training_stability_replay_fallback_trainable"),
        ("teacher_fallback_trainable_count", "guarded_formal_ppo_post_training_stability_replay_fallback_trainable"),
        ("diagnostic_trainable_count", "guarded_formal_ppo_post_training_stability_replay_diagnostic_trainable"),
        ("non_empty_gate_reason_trainable_count", "guarded_formal_ppo_post_training_stability_replay_gate_reason_trainable"),
        ("missing_observation_count", "guarded_formal_ppo_post_training_stability_replay_contract_invalid"),
        ("missing_log_prob_count", "guarded_formal_ppo_post_training_stability_replay_contract_invalid"),
        ("missing_value_count", "guarded_formal_ppo_post_training_stability_replay_contract_invalid"),
        ("invalid_action_mask_count", "guarded_formal_ppo_post_training_stability_replay_contract_invalid"),
        ...
        ("controlled_regression_count", "guarded_formal_ppo_post_training_stability_replay_controlled_regression"),
        ("controlled_safety_regression_count", "guarded_formal_ppo_post_training_stability_replay_controlled_regression"),
        ("controlled_contract_regression_count", "guarded_formal_ppo_post_training_stability_replay_controlled_regression"),
        ("controlled_path_risk_regression_count", "guarded_formal_ppo_post_training_stability_replay_controlled_regression"),
        ("controlled_source_selection_regression_count", "guarded_formal_ppo_post_training_stability_replay_controlled_regression"),
    ):
        if _int_value_or_default(summary.get(field), 0) > 0:
            _append_reason(blockers, reason)
    if _float_value_or_default(summary.get("teacher_agreement_rate"), 0.0) < 0.95:
        _append_reason(
            blockers,
            "guarded_formal_ppo_post_training_stability_replay_teacher_alignment_insufficient",
        )
    if summary.get("post_training_holdout_status") != "passed" or summary.get("post_training_canary_status") != "passed":
        _append_reason(blockers, "guarded_formal_ppo_post_training_stability_replay_gate_failed")
    if summary.get("publishes_checkpoint") is True:
        _append_reason(blockers, "limited_ppo_update_checkpoint_publication_claimed")
    if summary.get("replaces_default_policy") is True:
        _append_reason(blockers, "limited_ppo_update_default_policy_replacement_claimed")
    ...
    return {
        ...
        "replay_collector_trainable_transition_count": replay_trainable,
        "teacher_agreement_rate": _float_value_or_default(
            summary.get("teacher_agreement_rate"),
            0.0,
        ),
...
def _guarded_experimental_policy_staged_release_canary_preflight_readiness(
    summary: dict[str, Any],
) -> dict[str, Any]:
    empty = {
        "present": False,
        ...
        "training_blockers": [],
        "next_required_change": None,
        "staged_release_canary_preflight_verdict": None,
        "canary_eligible_activation_count": 0,
        "controlled_regression_count": 0,
    }
    if not summary:
        return empty
    ...
    blockers: list[str] = []
    eligible_count = _int_value_or_default(
        summary.get("canary_eligible_activation_count"),
        0,
    )
    max_activation_count = _int_value_or_default(
        summary.get("max_canary_control_activation_count"),
        16,
    )
    traffic_fraction = _float_value_or_default(summary.get("canary_traffic_fraction"), 1.0)
    if summary.get("status") != "passed" or _string_list(summary.get("reason_codes")):
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_canary_preflight_not_passed",
        )
    if (
        summary.get("staged_release_canary_preflight_verdict")
    ...
    if not summary.get("source_staged_release_trial_summary"):
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_canary_preflight_inputs_missing",
        )
    for field in (
        "manifest",
        "eligibility_ledger",
        ...
        if not summary.get(field):
            _append_reason(
                blockers,
                "guarded_experimental_policy_staged_release_canary_preflight_artifacts_missing",
            )
    if summary.get("staged_canary_enabled") is not False:
        _append_reason(
            blockers,
    ...
    if summary.get("default_policy_authoritative") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_canary_preflight_default_not_authoritative",
        )
    if traffic_fraction <= 0.0 or traffic_fraction > 0.01:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_canary_preflight_traffic_fraction_out_of_bounds",
        )
    if eligible_count < 1 or eligible_count > max_activation_count:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_canary_preflight_activation_count_out_of_bounds",
        )
    for field, reason in (
        ("diagnostic_fallback_rejected_canary_eligible_count", "guarded_experimental_policy_staged_release_canary_preflight_diagnostic_eligible"),
        ("missing_observation_canary_eligible_count", "guarded_experimental_policy_staged_release_canary_preflight_missing_observation"),
        ("non_finite_canary_eligible_count", "guarded_experimental_policy_staged_release_canary_preflight_non_finite"),
        ("controlled_regression_canary_eligible_count", "guarded_experimental_policy_staged_release_canary_preflight_controlled_regression"),
    ):
        if _int_value_or_default(summary.get(field), 0) > 0:
            _append_reason(blockers, reason)
    for field in (
    ...
    for field, reason in (
        ("kill_switch_audit_passed", "guarded_experimental_policy_staged_release_canary_preflight_kill_switch_failed"),
        ("rollback_audit_passed", "guarded_experimental_policy_staged_release_canary_preflight_rollback_failed"),
        ("telemetry_audit_passed", "guarded_experimental_policy_staged_release_canary_preflight_telemetry_failed"),
        ("automatic_downgrade_audit_passed", "guarded_experimental_policy_staged_release_canary_preflight_automatic_downgrade_failed"),
        ("canary_budget_audit_passed", "guarded_experimental_policy_staged_release_canary_preflight_budget_failed"),
        ("operator_approval_audit_passed", "guarded_experimental_policy_staged_release_canary_preflight_operator_approval_failed"),
    ):
        if summary.get(field) is not True:
            _append_reason(blockers, reason)
    if summary.get("default_policy_unchanged") is False:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_canary_preflight_default_policy_changed",
        )
    if summary.get("runs_staged_release_canary_preflight") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_canary_preflight_not_run",
        )
    if summary.get("runs_online_canary") is True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_canary_preflight_online_canary_started",
        )
    if summary.get("runs_new_ppo_update") is True:
        _append_reason(blockers, "formal_ppo_update_unexpected")
    if summary.get("publishes_checkpoint") is True:
    ...
    return {
        ...
        "staged_release_canary_preflight_verdict": summary.get(
        ...
        "automatic_downgrade_audit_passed": summary.get("automatic_downgrade_audit_passed") is True,
        "canary_budget_audit_passed": summary.get("canary_budget_audit_passed") is True,
        "operator_approval_audit_passed": summary.get("operator_approval_audit_passed") is True,
    }


def _analyze_guarded_experimental_policy_staged_release_trial_stage_only(
    *,
    batch_root: Path,
    staged_trial_path: Path,
    config: dict[str, Any],
    repo_root: Path,
) -> dict[str, Any]:
    reason_codes: list[str] = []
    source_summaries: dict[str, Any] = {}
    staged_trial_summary = _load_source(
        staged_trial_path,
        label="guarded_experimental_policy_staged_release_trial_summary",
        expected_schema=GUARDED_EXPERIMENTAL_POLICY_STAGED_RELEASE_TRIAL_SCHEMA_VERSION,
        repo_root=repo_root,
        reason_codes=reason_codes,
        source_summaries=source_summaries,
    )
    current_git = _git_snapshot(repo_root)
    if staged_trial_summary:
        _inspect_git(
            staged_trial_summary,
            label="guarded_experimental_policy_staged_release_trial_summary",
            current_git=current_git,
            config=config,
            reason_codes=reason_codes,
        )
    readiness = _guarded_experimental_policy_staged_release_trial_readiness(
        staged_trial_summary
    )
    blockers = list(readiness["training_blockers"])
    training_readiness_status = (
    ...
    return {
        ...
        "training_blockers": blockers,
        "recommended_next_action": recommended_next_action,
        "guarded_experimental_policy_staged_release_trial_readiness": readiness,
        "git_provenance": {"current": current_git, "current_matches_sources": not reason_codes},
    }


def _guarded_experimental_policy_staged_release_trial_readiness(
    summary: dict[str, Any],
) -> dict[str, Any]:
    empty = {
        "present": False,
        ...
        "training_blockers": [],
        "next_required_change": None,
        "staged_release_trial_verdict": None,
    ...
    blockers: list[str] = []
    activation_count = _int_value_or_default(
        summary.get("experimental_control_activation_count"),
        0,
    )
    max_activation_count = _int_value_or_default(
        summary.get("max_experimental_control_activation_count"),
        64,
    )
    if summary.get("status") != "passed" or _string_list(summary.get("reason_codes")):
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_not_passed",
        )
    if summary.get("staged_release_trial_verdict") != "eligible_for_guarded_staged_release_canary":
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_not_eligible",
        )
    if not summary.get("source_staged_release_preflight_summary"):
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_inputs_missing",
        )
    for field in (
        "activation_ledger",
        "controlled_regression_audit",
        "fallback_rejection_report",
        "kill_switch_drill",
        "rollback_drill",
        "telemetry_drill",
    ):
        if not summary.get(field):
            _append_reason(
                blockers,
                "guarded_experimental_policy_staged_release_trial_artifacts_missing",
            )
    if summary.get("staged_release_enabled") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_not_enabled",
        )
    if summary.get("default_policy_authoritative") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_default_not_authoritative",
        )
    if activation_count < 1 or activation_count > max_activation_count:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_activation_count_out_of_bounds",
        )
    for field, reason in (
        ("diagnostic_fallback_rejected_control_activation_count", "guarded_experimental_policy_staged_release_trial_diagnostic_control_activated"),
        ("missing_observation_count", "guarded_experimental_policy_staged_release_trial_missing_observation"),
        ("missing_log_prob_count", "guarded_experimental_policy_staged_release_trial_missing_log_prob_or_value"),
        ("missing_value_count", "guarded_experimental_policy_staged_release_trial_missing_log_prob_or_value"),
        ("invalid_action_mask_count", "guarded_experimental_policy_staged_release_trial_invalid_action_mask"),
        ("non_finite_logits_count", "guarded_experimental_policy_staged_release_trial_non_finite"),
        ("non_finite_log_prob_count", "guarded_experimental_policy_staged_release_trial_non_finite"),
        ("non_finite_value_count", "guarded_experimental_policy_staged_release_trial_non_finite"),
        ("non_finite_reward_count", "guarded_experimental_policy_staged_release_trial_non_finite"),
        ("missing_observation_control_activation_count", "guarded_experimental_policy_staged_release_trial_missing_observation"),
        ("non_finite_control_activation_count", "guarded_experimental_policy_staged_release_trial_non_finite"),
    ):
        if _int_value_or_default(summary.get(field), 0) > 0:
            _append_reason(blockers, reason)
    for field in (
    ...
    if summary.get("kill_switch_drill_passed") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_kill_switch_failed",
        )
    if _int_value_or_default(summary.get("post_kill_switch_activation_count"), 0) != 0:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_kill_switch_failed",
        )
    if summary.get("rollback_drill_passed") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_rollback_failed",
        )
    if summary.get("telemetry_drill_passed") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_telemetry_failed",
        )
    if summary.get("default_policy_unchanged") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_default_policy_changed",
        )
    if summary.get("connects_real_executor") is True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_real_executor_connected",
        )
    if summary.get("runs_staged_release_trial") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_staged_release_trial_not_run",
        )
    if summary.get("runs_new_ppo_update") is True:
        _append_reason(blockers, "formal_ppo_update_unexpected")
    if summary.get("publishes_checkpoint") is True:
    ...
    return {
        ...
        "staged_release_trial_verdict": summary.get("staged_release_trial_verdict"),
        ...
        "kill_switch_drill_passed": summary.get("kill_switch_drill_passed") is True,
        "rollback_drill_passed": summary.get("rollback_drill_passed") is True,
        "telemetry_drill_passed": summary.get("telemetry_drill_passed") is True,
...
def _analyze_guarded_experimental_policy_shadow_release_trial_stage_only(
    *,
    ...
    reason_codes: list[str] = []
    source_summaries: dict[str, Any] = {}
    shadow_trial_summary = _load_source(
        shadow_trial_path,
        label="guarded_experimental_policy_shadow_release_trial_summary",
        expected_schema=GUARDED_EXPERIMENTAL_POLICY_SHADOW_RELEASE_TRIAL_SCHEMA_VERSION,
        repo_root=repo_root,
        reason_codes=reason_codes,
        source_summaries=source_summaries,
    )
    current_git = _git_snapshot(repo_root)
    if shadow_trial_summary:
        _inspect_git(
    ...
    readiness = _guarded_experimental_policy_shadow_release_trial_readiness(
        shadow_trial_summary
    )
    blockers = list(readiness["training_blockers"])
    training_readiness_status = (
    ...
    return {
        ...
        "training_blockers": blockers,
        "recommended_next_action": recommended_next_action,
        "guarded_experimental_policy_shadow_release_trial_readiness": readiness,
        "git_provenance": {"current": current_git, "current_matches_sources": not reason_codes},
    }


def _guarded_experimental_policy_shadow_release_trial_readiness(
    summary: dict[str, Any],
) -> dict[str, Any]:
    empty = {
        "present": False,
        ...
        "training_blockers": [],
        "next_required_change": None,
        "shadow_release_trial_verdict": None,
        "shadow_step_count": 0,
        "unique_shadow_context_count": 0,
        "controlled_regression_count": 0,
    }
    ...
    if summary.get("status") != "passed" or _string_list(summary.get("reason_codes")):
        _append_reason(
            blockers,
            "guarded_experimental_policy_shadow_release_trial_not_passed",
        )
    if (
        summary.get("shadow_release_trial_verdict")
    ...
    for field in ("install_canary_summary", "multihorizon_shadow_summary"):
        if not summary.get(field):
            _append_reason(
                blockers,
                "guarded_experimental_policy_shadow_release_trial_inputs_missing",
            )
    for field in (
        "runtime_manifest",
        "step_comparison",
        ...
        if not summary.get(field):
            _append_reason(
                blockers,
                "guarded_experimental_policy_shadow_release_trial_artifacts_missing",
            )
    ...
    if (
        ...
        _append_reason(
            blockers,
            "guarded_experimental_policy_shadow_release_trial_checkpoint_hash_mismatch",
        )
    if package_size <= 0 or package_size != consumer_size:
        _append_reason(
            blockers,
            "guarded_experimental_policy_shadow_release_trial_checkpoint_size_mismatch",
        )
    if _int_value_or_default(summary.get("shadow_step_count"), 0) < 256:
        _append_reason(
            blockers,
            "guarded_experimental_policy_shadow_release_trial_step_count_below_threshold",
        )
    if _int_value_or_default(summary.get("unique_shadow_context_count"), 0) < 256:
        _append_reason(
            blockers,
            "guarded_experimental_policy_shadow_release_trial_unique_context_below_threshold",
        )
    if summary.get("shadow_policy_takes_control") is not False:
        _append_reason(
            blockers,
            "guarded_experimental_policy_shadow_release_trial_takes_control",
        )
    for field, reason in (
        ("missing_observation_count", "guarded_experimental_policy_shadow_release_trial_missing_observation"),
        ("missing_log_prob_count", "guarded_experimental_policy_shadow_release_trial_missing_log_prob_or_value"),
        ("missing_value_count", "guarded_experimental_policy_shadow_release_trial_missing_log_prob_or_value"),
        ("invalid_action_mask_count", "guarded_experimental_policy_shadow_release_trial_invalid_action_mask"),
        ("non_finite_logits_count", "guarded_experimental_policy_shadow_release_trial_non_finite"),
        ("non_finite_log_prob_count", "guarded_experimental_policy_shadow_release_trial_non_finite"),
        ("non_finite_value_count", "guarded_experimental_policy_shadow_release_trial_non_finite"),
        ("non_finite_reward_count", "guarded_experimental_policy_shadow_release_trial_non_finite"),
    ):
        if _int_value_or_default(summary.get(field), 0) > 0:
            _append_reason(blockers, reason)
    for field in (
    ...
    if summary.get("rollback_default_audit_passed") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_shadow_release_trial_rollback_default_failed",
        )
    if summary.get("default_policy_unchanged") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_shadow_release_trial_default_policy_changed",
        )
    if summary.get("runs_shadow_release_trial") is not True:
        _append_reason(
            blockers,
            "guarded_experimental_policy_shadow_release_trial_not_run",
        )
    if summary.get("runs_new_ppo_update") is True:
        _append_reason(blockers, "formal_ppo_update_unexpected")
    if summary.get("publishes_checkpoint") is True:
    ...
    return {
        ...
        "shadow_release_trial_verdict": summary.get("shadow_release_trial_verdict"),
        "shadow_step_count": _int_value_or_default(summary.get("shadow_step_count"), 0),
        "unique_shadow_context_count": _int_value_or_default(
            summary.get("unique_shadow_context_count"),
            0,
        ...
        "shadow_policy_takes_control": summary.get("shadow_policy_takes_control") is True,
        "shadow_rejection_diagnostic_count": _int_value_or_default(
            summary.get("shadow_rejection_diagnostic_count"),
            0,
        ),
        "shadow_fallback_diagnostic_count": _int_value_or_default(
            summary.get("shadow_fallback_diagnostic_count"),
            0,
        ),
    }
...
def analyze_policy_training_readiness_review(
    *,
    ...
    quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path: Path | None = None,
    ...
    quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_required: bool = False,
    ...
    if quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path is None:
        quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path = (
            batch_root
            / "quasi-real-guarded-formal-ppo-candidate-selection-long-horizon-holdout-summary.json"
        )
    ...
    quasi_real_guarded_formal_candidate_selection_long_horizon_only_mode = (
        quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_required
        and not any(
            (
                anchor_candidate_required,
                anchor_contract_required,
    ...
    stage_isolated_mode = (
        ...
        or quasi_real_guarded_formal_candidate_selection_long_horizon_only_mode
    )
    if (
        guarded_only_mode
        or return_aligned_only_mode
        ...
        isolated_label = (
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_only"
            if quasi_real_guarded_formal_candidate_selection_long_horizon_only_mode
            else
        ...
        if not quasi_real_guarded_formal_candidate_selection_long_horizon_only_mode:
            quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path = _isolated_path(
                quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path,
                "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_summary",
            )
    ...
    quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout = _load_optional_source(
        quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path,
        label="quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_summary",
        expected_schema=(
            QUASI_REAL_GUARDED_FORMAL_PPO_CANDIDATE_SELECTION_LONG_HORIZON_HOLDOUT_SCHEMA_VERSION
        ),
        repo_root=repo_root,
        reason_codes=reason_codes,
        source_summaries=source_summaries,
        required=quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_required,
    )
    ...
    if _fail_on_input_failure(config):
        for label, payload in (
            ...
            (
                "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_summary",
                quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout,
            ),
    ...
    if quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout:
        source_git_matches.append(
            _inspect_git(
                quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout,
                label=(
    ...
    return {
        ...
        "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_summary_path": (
            _display_path(
                quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_path,
                repo_root,
            )
        ...
        "git_provenance": {
            ...
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout": (
                _public_git(quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout)
            ),
...
def _review_metrics(
    *,
    ...
    quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout: dict[str, Any],
    ...
    quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_readiness = (
        _quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_readiness(
            quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout
        )
    )
    ...
    for reason in quasi_real_guarded_formal_ppo_stability_holdout_readiness[
        ...
        _append_reason(training_blockers, reason)
    for reason in quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_readiness[
        "training_blockers"
    ]:
        _append_reason(training_blockers, reason)
    ...
    if hard_validation_failed:
        ...
        quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_readiness["present"]
        and quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_readiness["completed"]
    ):
        training_readiness_status = (
            QUASI_REAL_GUARDED_FORMAL_PPO_CANDIDATE_SELECTION_LONG_HORIZON_HOLDOUT_EVALUATED_ACTION
        )
...
def _quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_readiness(
    summary: dict[str, Any],
) -> dict[str, Any]:
    empty = {
        "present": False,
        ...
        "horizon": 0,
        "completed_long_horizon_episode_count": 0,
    }
    if not summary:
        return empty
    ...
    if summary.get("status") != "passed" or _string_list(summary.get("reason_codes")):
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_not_passed",
        )

    trainable_transition_count = _int_value_or_default(
        summary.get("input_trainable_transition_count"), 0
    )
    long_horizon_trainable_transition_count = _int_value_or_default(
        summary.get("long_horizon_trainable_transition_count"), 0
    )
    optimizer_train_transition_count = _int_value_or_default(
        summary.get("optimizer_train_transition_count"), 0
    )
    ...
    eligible_candidate_count = _int_value_or_default(
        summary.get("eligible_candidate_count"), 0
    )
    horizon = _int_value_or_default(summary.get("horizon"), 0)
    completed_episode_count = _int_value_or_default(
        summary.get("completed_long_horizon_episode_count"), 0
    )
    if trainable_transition_count != 684 or long_horizon_trainable_transition_count != 684:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_trainable_count_mismatch",
        )
    if optimizer_train_transition_count != 0:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_unexpected_optimizer_update",
        )
    if unique_trainable_context_count != 684:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_unique_context_count_mismatch",
        )
    if eligible_candidate_count <= 0 or summary.get("selected_candidate_from_stability_matrix") is not True:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_candidate_missing",
        )
    if summary.get("candidate_selection_reproducible") is not True:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_selection_not_reproducible",
        )
    if horizon < 10 or completed_episode_count <= 0:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_horizon_below_threshold",
        )
    for field, reason in (
        ("validation_trainable_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_split_leakage"),
        ("test_trainable_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_split_leakage"),
        ("fallback_trainable_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_fallback_trainable"),
        ("source_fallback_trainable_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_fallback_trainable"),
        ("teacher_fallback_trainable_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_fallback_trainable"),
        ("non_empty_gate_reason_trainable_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_gate_reason_trainable"),
        ("missing_observation_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_contract_invalid"),
        ("missing_log_prob_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_contract_invalid"),
        ("missing_value_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_contract_invalid"),
        ("non_finite_reward_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_non_finite"),
        ("non_finite_return_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_non_finite"),
        ("non_finite_advantage_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_non_finite"),
        ("loss_non_finite_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_non_finite"),
        ("non_finite_gradient_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_non_finite"),
        ("non_finite_long_horizon_return_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_non_finite"),
        ("non_finite_long_horizon_advantage_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_non_finite"),
        ("controlled_regression_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_controlled_regression"),
        ("train_controlled_regression_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_controlled_regression"),
        ("validation_controlled_regression_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_controlled_regression"),
        ("test_controlled_regression_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_controlled_regression"),
        ("controlled_safety_regression_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_controlled_regression"),
        ("controlled_contract_regression_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_controlled_regression"),
        ("controlled_path_risk_regression_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_controlled_regression"),
        ("controlled_source_selection_regression_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_controlled_regression"),
        ("family_regression_count", "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_family_regression"),
    ):
        if _int_value_or_default(summary.get(field), 0):
            _append_reason(blockers, reason)
    if _float_value_or_default(summary.get("max_old_log_prob_abs_error"), 0.0) > 1.0e-4:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_old_policy_reconstruction_error",
        )
    if _float_value_or_default(summary.get("max_old_value_abs_error"), 0.0) > 1.0e-4:
        _append_reason(
            blockers,
    ...
    if abs(_float_value_or_default(summary.get("max_abs_approx_kl"), 0.0)) > 0.25:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_kl_too_large",
        )
    if _float_value_or_default(summary.get("max_grad_norm_after_clip"), 0.0) > 1.0:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_grad_norm_too_large",
        )
    if _float_value_or_default(summary.get("min_parameter_l2_delta"), 0.0) <= 0.0:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_parameter_delta_missing",
        )
    if _float_value_or_default(summary.get("teacher_agreement_rate"), 0.0) < 0.95:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_teacher_alignment_insufficient",
        )
    if not summary.get("selection_audit") or not summary.get("candidate_manifest") or not summary.get("rollback_manifest"):
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_manifest_missing",
        )
    if summary.get("runs_formal_ppo_candidate_selection_long_horizon_holdout") is not True:
        _append_reason(
            blockers,
            "quasi_real_guarded_formal_ppo_candidate_selection_long_horizon_holdout_not_run",
        )
    if summary.get("runs_new_ppo_update") is True:
        _append_reason(blockers, "formal_ppo_update_unexpected")
    if summary.get("publishes_checkpoint") is True:
    ...
    return {
        ...
        "horizon": horizon,
        "completed_long_horizon_episode_count": completed_episode_count,
    }
