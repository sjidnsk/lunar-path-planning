...
def git_snapshots_match(left: dict[str, Any], right: dict[str, Any], *, submodules: Iterable[str] = DEFAULT_SUBMODULES) -> bool:
    ...
    for name in submodules:
        ...
        if _declares_cleanliness(left_modules.get(name)):
            if not _repo_is_clean(left_modules.get(name)) or not _repo_is_clean(right_modules.get(name)):
                return False
    return True
...
def inspect_source_git_provenance(
    ...
    *,
    ...
    elif not git_snapshots_match(source_current, current_git, submodules=submodules):
