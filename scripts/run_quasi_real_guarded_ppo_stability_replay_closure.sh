...
OUTPUT="${OUTPUT:-outputs/path_feedback_batch_quasi_real_guarded_ppo_stability_replay_v1}"
CONFIG="${CONFIG:-configs/quasi_real_guarded_ppo_stability_replay_v1.json}"

rm -rf "$REPO_ROOT/$OUTPUT"

PYTHON="$PYTHON_BIN" bash "$SCRIPT_DIR/run_quasi_real_guarded_ppo_stability_replay.sh" \
  --freeze-root "$FREEZE" \
